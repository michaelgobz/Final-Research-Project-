...
```{r ADL model and test for stationality and Predicatble powers}
 GDP_adl <- dynlm(ts(GDP_level) ~ L(ts(GDP_level)) + L(ts(FDI_stock_level)) + L(ts(FDI_inflow_level)) + L(ts(NFA_level)) + L(ts(REER_level)))
...
c("L(ts(FDI_stock_level))=0" ,"L(ts(FDI_inflow_level))=0", "L(ts(NFA_level))=0", "L(ts(REER_level))=0"),
vcov. = sandwich)

linearHypothesis(GDP_adl,
c("L(ts(FDI_inflow_level))=0"),
vcov. = sandwich)
linearHypothesis(GDP_adl,
c("L(ts(FDI_stock_level))=0"),
vcov. = sandwich)
