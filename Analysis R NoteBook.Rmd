...
GDP_Growth <- xts(log(GDP/lag(GDP)))
GDP_Growth
...
GDP_armod <- dynlm(ts(GDP_level) ~ L(ts(GDP_level)) + L(ts(GDP_level), 2) + L(ts(GDP_level), 3) + L(ts(GDP_level), 4) + L(ts(GDP_level), 5)+ L(ts(GDP_level), 6) +L(ts(GDP_level), 7))
...
acf(na.omit(GDP_Growth), lag.max = 4, plot= F)
pacf(na.omit(GDP_Growth),lag.max = 4, plot= F)
...
# Calculate the ACF and PACF 
acf(na.omit(FDI_Growth), lag.max = 4, plot= T)
pacf(na.omit(FDI_Growth),lag.max = 4, plot= T)
...
# Calculate the ACF and PACF 
acf(na.omit(FDI_Stock_Growth), lag.max = 4, plot= T)
pacf(na.omit(FDI_Stock_Growth),lag.max = 4, plot= T)
...
# Calculate the ACF and PACF 
acf(na.omit(NFA_Growth), lag.max = 4, plot= T)
pacf(na.omit(NFA_Growth),lag.max = 4, plot= T)
...
# Calculate the ACF and PACF 
acf(na.omit(REER_Growth), lag.max = 4, plot= T)
pacf(na.omit(REER_Growth),lag.max = 4, plot= T)
...
```{r ADL model and test for stationality and Predicatble powers}
 GDP_adl <- dynlm(ts(GDP_level) ~ L(ts(GDP_level)) + L(ts(GDP_level),2) + L(ts(FDI_stock_level)) + L(ts(FDI_stock_level),2) + L(ts(FDI_inflow_level)) + L(ts(FDI_inflow_level),2) + L(ts(NFA_level)) + L(ts(NFA_level),2) + L(ts(REER_level)) + L(ts(REER_level),2))
